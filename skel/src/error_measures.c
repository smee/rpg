...
static double mae(const double *x, const double *y, R_len_t n) {
    double sum = 0.0;
    R_len_t i;
    for (i = 0; i < n; ++i) {
        sum += fabs(x[i] - y[i]);
...
static double sse(const double *x, const double *y, R_len_t n) {
    double sum = 0.0;
    R_len_t i;
    for (i = 0; i < n; ++i) {
...
static double ssse(const double *x, const double *y, R_len_t n) {
    R_len_t i;
    ...
    for (i = 0; i < n; ++i) {
        const double delta = x[i] - (a + b * y[i]);        
        sum += delta * delta;
    }
    return sum;
}
