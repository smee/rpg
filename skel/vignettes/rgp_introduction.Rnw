...
\def\sourcewerk{{\sf sourcewerk}\xspace}
...
\hypersetup{
  ...
  urlcolor = linkcol}
\title[\rgp]{%
\protect \includegraphics[width=3.2cm]{skel/vignettes/figures/logo_rgp.pdf}\\
A Friendly Introduction to RGP}
\date{4 March 2013}
\author[O. Flasch]{Oliver Flasch}
...
\begin{abstract}
  \noindent \finitestar is a full-stack solution for engineering optimization in the form of an
  easy-to-deploy appliance. By tightly integrating the leading Open Source tools for simulation,
  modelling, optimization, and product lifecycle management with powerful cluster hardware and
  \sourcewerk's innovative web-based user interface, \finitestar offers a level of integration,
  usability, flexibility, and cost-effectiveness unprecedented in the market of engineering
  optimization tools. \finitestar gets the technology right, so that you can concentrate
  on what matters most: Building great products.
\end{abstract}

This whitepaper provides a quick overview of \finitestar, the first full-stack engineering optimization
appliance. After section~\ref{sec:intro} gives a short introduction to typical questions engineering
optimization helps to answer, section~\ref{sec:process} describes the integrated optimization process
implemented in \finitestar. The unique concept of an appliance that assembles powerful hard- and software
components into an easy-to-deploy and easy-to-use package is detailed in section~\ref{sec:appliance}.
%Section~\ref{sec:applications} lists application areas from multiple industries.


\section{Engineering Optimization}\label{sec:intro}

\begin{marginfigure}
  %\includegraphics[width=1\linewidth]{figures/nozzle_variant_A_magU_3000x700.png}
  %\includegraphics[width=1\linewidth]{figures/nozzle_variant_A_ribbons_3000x700.png}
  %\includegraphics[width=1\linewidth]{figures/nozzle_variant_B_magU_3000x700.png}
  %\includegraphics[width=1\linewidth]{figures/nozzle_variant_A_ribbons_3000x700.png}
  \caption{Shape optimization of an injection nozzle: \finitestar integrates the parametric
  \cad kernel \opencascade, the computational fluid dynamics package \openfoam, and the
  optimization packages \dakota and \spot, enabling the efficient solution of complex shape
  optimization tasks based on user-supplied \cad models.}
  \label{fig:nozzle}
  \setfloatalignment{t}
\end{marginfigure}
...
research departments faced with questions like:
\begin{itemize}
\item {\em Sensitivity analysis:} Which parameters of an industrial process or product design
have the strongest effect on operating costs, quality, energy efficiency, or pollutant emissions?
\item {\em Process modelling:} What relations exist between process parameters and target
quantities such as quality, costs, and efficiency?
\item {\em Statistical analysis:} Are these relations causal connections or just accidental
correlations? Are correlations statistically significant? How can these relations be utilized
for process optimization?
\item {\em Design and process optimization:} Which setting of process or design parameters
...
\begin{figure}[h]
  %\includegraphics[width=1\linewidth]{figures/burner_CO2_1000x625.png}
  \caption{Prediction of pollutant concentrations in coal-fired power plants: The inclusion of
  combustion and pollutant emission models into the \openfoam framework for multiphysics simulation
  allows accurate modelling of pollutant concentrations. By coupling \openfoam with the optimization
  packages \dakota and \spot, \finitestar enables the automatic exploration of process parameter
  space in search of an optimal compromise between plant efficiency and pollutant emissions.}
  \label{fig:burner}
  \setfloatalignment{t}
\end{figure}


\section{Integrated Optimization Process}\label{sec:process}
...
\section{Full-Stack Appliance}\label{sec:appliance}

\begin{marginfigure}[-8.8cm]
  %\includegraphics[width=1\linewidth]{figures/nagare_screenshot.png}
  \caption{Integrated graphical user interface: \finitestar is operated through a
  modern web-based graphical user interface that offers fine-grained control
  over each step of an analysis or optimization task.}
  \label{fig:gui}
  \setfloatalignment{t}
\end{marginfigure}
...
%TODO add a section on application areas / preset modules


\section*{Imprint}
\begin{minipage}[t]{\linewidth}
\sourcewerk UG (haftungsbeschränkt)\\
Heiliger Weg~60\\
44135 Dortmund\\
Web: \url{sourcewerk.de}\\
Email: {\tt info@sourcewerk.de}\\
\vspace{0.5cm}
\small{\copyright~2011-13 \sourcewerk UG}\\
