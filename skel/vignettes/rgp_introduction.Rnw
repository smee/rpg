...
symbols. In other words, these three sets define the symbolic expression
search space.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.75\linewidth]{skel/vignettes/figures/best_versus_true_sine.pdf}
...
regression run. We choose a time budget of 2 minutes:
...
\bibliography{skel/vignettes/rgp_introduction}
