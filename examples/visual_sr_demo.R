...
progressMonitor1 <- function(pop, fitnessValues, fitnessFunction, stepNumber, evaluationNumber, bestFitness, timeElapsed) {
  ...
  if (stepNumber %% 50 == 0) {
    ...
    rescaledBestIndividual <- rescaleIndividual(bestIndividual, df1$y)
    message("current best individual (not rescaled):")
    message(sprintf(" %s", deparse(bestIndividual)))
    plotFunctions(list(Salutowicz1d, rescaledBestIndividual, bestIndividual), from = 0, to = 10, steps = 100,
